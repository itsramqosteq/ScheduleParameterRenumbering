...
namespace ScheduleParameterRenumbering
{
    ...
    public partial class Utility
    {
        ...
        public static void SetAlertColor(ElementId id, Document _doc, UIDocument _uidoc,Autodesk.Revit.DB.Color color = null, List<Element> Views = null)
        {

            if (color == null)
                color = new Autodesk.Revit.DB.Color(255, 0, 0);
            var patternCollector = new FilteredElementCollector(_doc);
        ...
        {
            XYZ clonePoint;
            XYZ startOrgin = null, endOrgin = null;
            GetStartEndPointForConduit(con, ref startOrgin, ref endOrgin);
            if (_crossLine != null)
            {
        ...
        public static string GetStringFromNumber(UIApplication uiApp, double number)
        {
            Document doc = uiApp.ActiveUIDocument.Document;
            return UnitFormatUtils.Format(doc.GetUnits(), UnitType.UT_Length, number, false, false);
            ////Uncomment below code for revit above 2021
            //return UnitFormatUtils.Format(doc.GetUnits(), Autodesk.Revit.DB.SpecTypeId.Length, number, false);
        }


        public static void GetProjectUnits(UIApplication uiApp, string value, out double asDouble, out string asString, bool hideMessageBox = false)
        {
            Document doc = uiApp.ActiveUIDocument.Document;
            UnitFormatUtils.TryParse(doc.GetUnits(), UnitType.UT_Length, value, out asDouble, out string message);
            asString = UnitFormatUtils.Format(doc.GetUnits(), UnitType.UT_Length, asDouble, true, false);
            ////Uncomment below code for revit above 2021
            //UnitFormatUtils.TryParse(doc.GetUnits(), Autodesk.Revit.DB.SpecTypeId.Length, value, out asDouble, out string message);
            //asString = UnitFormatUtils.Format(doc.GetUnits(), Autodesk.Revit.DB.SpecTypeId.Length, asDouble, true);
        }
